...
\usepackage{scalefnt} % math symbols
\usepackage{tikz}
\usetikzlibrary{calc}
...
\pagestyle{fancy}
\thispagestyle{empty}
\fancyhead[RO]{hemaclass.com}
\fancyhead[LE]{Steffen Falgreen et al.}
...
\email{Steffen Falgreen\correspondingauthor - sfl@rn.dk},
Anders Ellern Bilgrau\,$^{1,2}$
\email{Anders Ellern Bilgrau - a.bilgrau@rn.dk},
Tarec Christoffer El-Galaly\,$^{1}$
\email{Tarec Christoffer El-Galaly - tarec.galaly@gmail.com},
Julie S.\  B\o dker$^{1}$%
\email{ Julie S.\  B\o dker - j.boedker@rn.dk},
Alexander Schmitz$^{1}$%
\email{Alexander Schmitz - alex.schmitz@rn.dk},
Malene K.\ Kjeldsen$^{1}$%
\email{Malene K.\ Kjeldsen - makrp@rn.dk},
Hans E.\ Johnsen$^{1}$%
\email{Hans E.\ Johnsen - haej@rn.dk},
Karen Dybk\ae r$^{1}$%
\email{Karen Dybk\ae r - k.dybkaer@rn.dk},
Martin B\o gsted$^{1,2}$%
\email{Martin B\o gsted - martin.boegsted@rn.dk}
      }
...
\end{center}
\captionof{figure}{\textbf{Diagram of the workflow architecture.}}
...
The RMA pre-processing was carried out with the Bioconductor package \texttt{affy} \cite{Gautier2004}. %The code is open source and freely available for sharing, modification, and redistribution at GitHub (xxx).
...
\begin{Table}%[htb]
\small
\captionof{table}{\textbf{Overview of the GEO accession numbers for the used datasets.}}
\label{table:01}%
\begin{center}
...
\begin{Table}
\small
\captionof{table}{\textbf{Comparison of ABC/GCB classification performed using Wright's method and the established classifier based on cohort normalisation for both.} The first column shows the rate of agreement between the classifiers with 95\% CI. The second column shows the Cohen's $\kappa$ with 95\% CI.\label{tab:ABCGCBclassifier}}
\begin{center}
...
\begin{Figure}
\begin{center}
\includegraphics[width=0.975\textwidth]{DruglogitChep.pdf}
\end{center}
\captionof{figure}{\small\textbf{Comparison of logit probabilities for ABC/GCB and REGS classification obtained through cohort normalisation and hemaclass.com.} The areas marked with green indicate patients with similar classification between hemaclass.com and cohort based normalisation. Contrary, the areas marked with blue indicate misclassification. For ABC/GCB and REGS the grey areas indicate unclassified and intermediate resistant, respectively.}
\label{fig:ABCGCBDrug}
\end{Figure}

\begin{Figure}
\begin{center}
\includegraphics[width=0.975\textwidth]{BAGSlogitChep.pdf}
\end{center}
\captionof{figure}{\small\textbf{Comparison of logit probabilities for the BAGS classifier obtained through cohort normalisation and hemaclass.com.} The coloured regions in the figure correspond to a threshold probability of 0.5.}
\label{fig:Bagscorr}
\end{Figure}
\newpage
...
{\ifthenelse{\boolean{publ}}{\footnotesize}{\small}
 \bibliographystyle{bmc_article}  % Style BST file
  \bibliography{library2}}     % Bibliography file (usually '*.bib' )

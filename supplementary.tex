...
\subsection{Backgroud adjustment}
...
Furthermore, let $G_i$ denote the empirical cumulative distribution function (ECDF) of the probes $\{x_{ijk}\}_{jk}$ on the $i$'th cohort array and $F$ the ECDF of the across array averaged sample quantiles $\{\bar{x}_{\cdot (jk)}\}_{ij}$, where $\{x_{i(jk)}\}_{jk}$ is the order statistic of all probes on the i'th cohorte array. Then each data point is quantile normalized in the following way
\begin{equation*}
...
Let $x_{ijk}$ be the $\log_2$-transformed and background corrected reference data, where $i = 1,\dots,I_R$ index the arrays of the reference data, $j=1,\dots,J$  index the probe-sets, and $k=1,\dots,K_j$ index the probes. Assume $x_{ijk}$ has been RMA normalized as descrbied above. Similarly,  let $y_{ijk}$ be the log2-transformed and background corrected user supplied data, where $i = 1,\dots,I_U$ index the arrays of the user supplied data, $j=1,\dots,J$  index the probe-sets, and $k=1,\dots,K_j$ index the probes.

Furthermore, let G and H denote the ECDF of all the reference $\{x_{ijk}\}_{ijk}$ and user supplied data $\{y_{ijk}\}_{ijk}$, respectively. Finally, let $H^{-1}$ denote the sample quantile of type 4 based on the reference data set \citep{Hyndman1996}.
